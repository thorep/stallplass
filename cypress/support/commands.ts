...
Cypress.Commands.add('createStable', (options?: {
  name?: string
  addressQuery?: string
  descriptionLength?: number
  amenityCount?: number
  ...
  cy.contains('button', /Oslo gate|Oslo/i, { timeout: 10000 }).first().click({ force: true })
  ...
  cy.contains('button', 'Velg bilder').should('be.visible').click()
  cy.get('input[type="file"][accept*="image"]').first().selectFile('stable.jpg', { force: true })
  ...
  cy.get('[data-cy="save-stable-button"]').scrollIntoView().should('be.visible').click({ force: true })
...
Cypress.Commands.add('ensureStable', (options?: {
  addressQuery?: string
  descriptionLength?: number
  amenityCount?: number
}) => {
...
Cypress.Commands.add('createBox', (options?: {
  stableName?: string
  stableIndex?: number
  name?: string
  price?: number
  ...
  if (openModal) {
    if (typeof stableIndex === 'number') {
      cy.get('[data-cy^="stable-card-"]').eq(stableIndex).within(() => {
        cy.get('[data-cy="add-box-button"]').scrollIntoView().should('be.visible').click({ force: true })
      })
    } else if (stableName) {
      ...
      cy.get('[data-cy="add-box-button"]').first().scrollIntoView().should('be.visible').click({ force: true })
    }
  ...
  // Upload image
  cy.contains('button', 'Velg bilder').should('be.visible').click()
  cy.get('input[type="file"][accept*="image"]').first().selectFile(imagePath, { force: true })

  // Save and wait for modal to disappear
  cy.get('[data-cy="save-box-button"]').scrollIntoView().should('be.visible').click({ force: true })
  cy.get('[data-cy="box-management-form"]', { timeout: 30000 }).should('not.exist')
