...
export async function PUT(
  ...
  try {
    ...
    try { const { id } = await params; captureApiError({ error, context: 'admin_horse_discipline_put', route: '/api/admin/horse-disciplines/[id]', method: 'PUT', id }); } catch {}
...
export async function DELETE(
  ...
  try {
    ...
    try { const { id } = await params; captureApiError({ error, context: 'admin_horse_discipline_delete', route: '/api/admin/horse-disciplines/[id]', method: 'DELETE', id }); } catch {}
    if ((error as { code?: string })?.code === 'P2025') {
      return NextResponse.json({ error: 'Discipline not found' }, { status: 404 });
    }
