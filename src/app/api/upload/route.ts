...
export async function POST(request: NextRequest) {
  ...
  const contentLengthMB = contentLength ? Math.round((parseInt(contentLength) / (1024 * 1024)) * 100) / 100 : null;
  ...
  try {
    ...
    if (hasMultipleFiles) {
      ...
      return NextResponse.json({ 
        ...
        details: 'This endpoint only accepts one file at a time. Please upload files individually.',
        fileCount: allFiles.length,
        totalSizeMB
      }, { status: 400 });
    }
    ...
    if (file.size > IMAGE_CONSTRAINTS.MAX_FILE_SIZE) {
      ...
      return NextResponse.json({ 
        ...
        details: `File size ${Math.round(fileSizeMB * 100) / 100}MB exceeds the ${maxSizeMB}MB limit`,
    ...
    if (!uploadResponse.ok) {
      ...
      const statusCode = uploadResponse.status === 403 ? 403 : 
                        uploadResponse.status === 413 ? 413 : 500;
