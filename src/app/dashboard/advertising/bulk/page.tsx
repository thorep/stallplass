...
import { useGetPublicDiscounts, useCalculatePricing, useGetPublicBoxQuantityDiscounts } from '@/hooks/usePricing';
...
function BulkAdvertisingPageContent() {
  ...
  const { data: pricing, isLoading: pricingLoading } = useCalculatePricing(selectedBoxes.length, months);
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              ...
              <div className="space-y-3 max-h-64 overflow-y-auto">
                {selectedBoxes.map(box => (
                  <div key={box.id} className="flex items-center justify-between text-sm border-b border-gray-100 pb-2">
                    <div>
        ...
        {quantityDiscounts && quantityDiscounts.length > 0 && (
          <div className="mt-6 bg-green-50 rounded-lg p-6">
            <h3 className="font-medium text-green-900 mb-2">💰 Mengderabatter - spar mer med flere bokser:</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {quantityDiscounts.map((discount: { minBoxes: number; maxBoxes: number | null; discountPercentage: number }, index: number) => {
                const currentBoxCount = selectedBoxes.length;
                const isCurrentTier = currentBoxCount >= discount.minBoxes && (discount.maxBoxes === null || currentBoxCount <= discount.maxBoxes);
