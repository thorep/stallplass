...
interface UseRealTimeRentalOptions {
  ownerId?: string;
  stableId?: string;
  enabled?: boolean;
 ...
 * Uses 'rentals' table with English column names
 */
export function useRealTimeRental(options: UseRealTimeRentalOptions = {}) {
  const { 
    ownerId, 
    enabled = true, 
    trackAnalysis = true, 
      ...
      if (activeRentals.length > 1) {
        activeRentals.forEach(rental => {
          conflicts.push({
            ...
            boxId,
            severity: 'CRITICAL',
            description: `Multiple active rentals for the same box`,
            suggestedSolution: 'Manual review of rental agreements required',
  ...
  // Helper functions
  const updateAnalysis = useCallback((rental: Rental, eventType: 'INSERT' | 'UPDATE' | 'DELETE') => {
    setAnalysis(prev => {
      if (!prev) return null;

      const updates: Partial<RentalAnalysis> = {};
  ...
  const checkForNewConflicts = useCallback(async (rental: Rental) => {
    ...
    if (overlappingRentals.length > 0) {
      const conflict: RentalConflict = {
        ...
        boxId: rental.box_id,
        severity: 'CRITICAL',
        description: `Double booking detected for box ${rental.box_id}`,
        suggestedSolution: 'Contact both parties to resolve the conflict',
        autoResolvable: false,
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (trackAnalysis) {
          ...
          setAnalysis({
            ...
            monthlyIncome: stats.monthlyRevenue,
            ...
            cancellationRate: 0, // Calculate from rental data
            recentTrends: {
    ...
    loadInitialData();
  }, [ownerId, enabled, trackAnalysis, detectConflicts, detectRentalConflicts]);
  ...
  useEffect(() => {
    ...
    const handleRentalChange = (rental: Rental, eventType: 'INSERT' | 'UPDATE' | 'DELETE') => {
      ...
      if (eventType === 'UPDATE' || eventType === 'INSERT') {
        const event: RentalLifecycleEvent = {
          ...
          timestamp: new Date(),
          triggeredBy: 'system',
          description: `Rental ${eventType === 'INSERT' ? 'created' : 'updated'}`,
          metadata: {
      ...
      // Update analysis
      if (trackAnalysis) {
        updateAnalysis(rental, eventType);
      }
    ...
    const handleStatusChange = (rental: Rental) => {
      const event: RentalLifecycleEvent = {
        ...
        description: `Status changed to ${rental.status}`
      };
    ...
    const handleNewRequests = (rental: RentalWithRelations) => {
      ...
      const event: RentalLifecycleEvent = {
        ...
        description: 'New rental request received'
      };
    ...
    const requestsChannel = subscribeToNewRentalRequests(ownerId, handleNewRequests);
    ...
    return () => {
      ...
      if (requestsChannelRef.current) {
        unsubscribeFromRentalChannel(requestsChannelRef.current);
        requestsChannelRef.current = null;
      }
    };
  }, [ownerId, enabled, trackAnalysis, detectConflicts, updateAnalysis, checkForNewConflicts]);
  ...
  }, [ownerId, trackAnalysis, detectConflicts, detectRentalConflicts]);

  const resolveConflict = useCallback((conflictId: string, solution: string) => {
    ...
    const event: RentalLifecycleEvent = {
      ...
      description: `Conflict resolved: ${solution}`,
      metadata: {
        reason: solution,
    ...
    const handleStatusChange = (updatedRental: Rental) => {
      if (updatedRental.id === rentalId) {
        ...
        const event: RentalLifecycleEvent = {
          ...
          timestamp: new Date(),
          triggeredBy: 'status_update',
          description: `Status changed from ${previousStatus} to ${updatedRental.status}`
        };
 ...
 */
export function useRealTimeRenterRentals(renterId: string, enabled = true) {
    ...
    const handleRentalUpdate = (rental: Rental, eventType: 'INSERT' | 'UPDATE' | 'DELETE') => {
      if (rental.rider_id === renterId) {
        setMyRentals(prev => {
          ...
          return prev; // New rental would need full data fetch
        });
        ...
        const event: RentalLifecycleEvent = {
          ...
          description: `Rental updated to status: ${rental.status}`,
          metadata: {
            reason: `Your rental has been updated to status: ${rental.status}`
          }
        };
