...
export const horseSizeSchema = z.enum(
  ["KATEGORI_4", "KATEGORI_3", "KATEGORI_2", "KATEGORI_1", "UNDER_160", "SIZE_160_170", "OVER_170"],
  {
...
export const createHorseSaleSchema = z.object({
  ...
  breedId: z.string().uuid("Ugyldig rase ID"),

  disciplineId: z.string().uuid("Ugyldig disiplin ID"),
  ...
  latitude: z.number().min(-90, "Ugyldig breddegrad").max(90, "Ugyldig breddegrad").optional(),

  longitude: z.number().min(-180, "Ugyldig lengdegrad").max(180, "Ugyldig lengdegrad").optional(),

  countyId: z.string().uuid("Ugyldig fylke ID").optional(),

  municipalityId: z.string().uuid("Ugyldig kommune ID").optional(),
...
export const horseSalesSearchSchema = z.object({
  ...
  sortBy: z
    .enum(["newest", "oldest", "price_low", "price_high", "name_asc", "name_desc"])
    .default("newest"),
});
