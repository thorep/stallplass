...
function AdvertisingPromotionCard({ className = "" }: AdvertisingPromotionCardProps) {
  return (
    <Link
      href="/annonsering"
      className={`block bg-white rounded-lg shadow-sm border-2 border-dashed border-violet-200 overflow-hidden transition-all duration-300 hover:shadow-lg hover:scale-[1.02] hover:border-violet-300 cursor-pointer group ${className}`}
    >
      {/* Mobile-first: Stack layout matching BoxListingCard */}
      <div className="flex flex-col md:flex-row">
        {/* Left side - Visual/Icon area */}
        <div className="relative md:w-1/3">
          <div className="relative h-48 md:h-full w-full bg-gradient-to-br from-violet-50 via-purple-50 to-violet-100 group-hover:from-violet-100 group-hover:to-purple-100 transition-colors flex items-center justify-center">
            {/* Background pattern */}
            <div className="absolute inset-0 opacity-10">
              <div className="absolute top-4 right-4 w-12 h-12 border-2 border-violet-400 rounded-full"></div>
              <div className="absolute bottom-6 left-6 w-6 h-6 border border-purple-400 rounded-full"></div>
              <div className="absolute top-1/3 left-1/4 w-2 h-2 bg-violet-400 rounded-full"></div>
              <div className="absolute bottom-1/3 right-1/4 w-1.5 h-1.5 bg-purple-400 rounded-full"></div>
            </div>
            
            {/* Icon */}
            <div className="relative z-10">
              <div className="w-16 h-16 bg-violet-600 rounded-full flex items-center justify-center shadow-lg group-hover:bg-violet-700 transition-colors">
          ...
          <div className="absolute top-3 right-3">
            <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-violet-200 text-violet-800">
              Annonse
            </span>
          </div>
        </div>
        ...
        <div className="flex-1 p-4 md:p-6">
          <div className="h-full flex flex-col justify-center">
            {/* Main text */}
            <h3 className="text-xl md:text-2xl font-bold text-violet-900 mb-3 leading-tight">
            ...
            {/* Call to action */}
            <div className="flex items-center text-violet-700 font-semibold group-hover:text-violet-800 transition-colors">
