...
const UnifiedImageUpload = forwardRef<UnifiedImageUploadRef, UnifiedImageUploadProps>(({
  ...
  useImperativeHandle(ref, () => ({
    uploadPendingImages: async (): Promise<string[]> => {
      ...
      try {
        console.log(`[UnifiedImageUpload] Form uploading ${stagedImages.length} images for ${entityType}`);
        ...
        console.log(`[UnifiedImageUpload] Form successfully uploaded ${uploadedUrls.length} images`);
        
        return allUploadedUrls;
        
      } catch (uploadError) {
        console.error('[UnifiedImageUpload] Form upload failed:', uploadError);
        setError(`Feil ved opplasting: ${uploadError instanceof Error ? uploadError.message : 'Ukjent feil'}`);
        throw uploadError;
      } finally {
  ...
  const uploadStagedImages = async (closeModalAfter = false) => {
    ...
    try {
      console.log(`[UnifiedImageUpload] Uploading ${stagedImages.length} images for ${entityType}`);
  ...
  const deleteImage = async (index: number) => {
    ...
    const uploadedUrls = newImageData
      .filter(img => img.isUploaded)
      .map(img => img.url);
    onChange(uploadedUrls);
  };
  ...
  const content = (
    <div className={`space-y-6 ${className}`}>
      ...
      {imageData.filter(img => img.isUploaded).length < maxImages && (
        <FileInput />
      )}
