...
import { useMessages, useSendMessage, useMarkMessagesAsRead, type MessageWithSender } from '@/hooks/useChat';
...
export function ConversationChat({ conversation }: ConversationChatProps) {
  ...
  const otherUser = conversation.stable?.ownerId === user?.id 
    ? conversation.profile // If we're the stable owner, other user is the one who started conversation
    : { // If we're the conversation starter, other user is the stable owner
        id: conversation.stable?.ownerId || '',
        nickname: conversation.stable?.profiles?.nickname || 'Stalleier', // Will need to get this from API
        avatar: conversation.stable?.profiles?.avatar || undefined
      };
        ...
        <ChatContainer>
          <ConversationHeader>
            <ConversationHeader.Content 
              userName={otherUser?.nickname || 'Bruker'}
              info={
                ...
                <div key={msg.id}>
                  ...
                  <Message
                    model={{
                      ...
                      sender: msg.sender?.nickname || 'Bruker finnes ikke',
